...
## Fix
Run `payload migrate:create` to generate the initial migration from the schema (covering `admins`, `media`, `services`, `lawns`, and all related Payload system tables), commit `src/migrations/*`, and run `payload migrate` in the deploy step (and seed the Kryscar tenant + services). Medium severity (blocks first prod deploy), low effort.
